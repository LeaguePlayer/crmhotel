...
<?$hotel_category = Hotels::model()->findByPk($id_hotel)?>
 <?
            ...
            $get_time = HotelOrder::model()->find(array('condition'=>"id_hotel = $id_hotel and date(date_stay_finish) = '$today'",'order'=>'date_stay_finish DESC'));
...
<?php echo CHtml::checkBox('howhous','1',array('class'=>'hiden')); ?>
	...
	<div class="row">
		<?php echo CHtml::label('Стоимость',''); ?>
        <?echo CHtml::textField('price_per_day',$hotel_category->cost,array('size'=>10,'maxlength'=>10))?>
    ...
    <div class="row igoro">
   	...
   	<div class="row">
        ...
        <?php echo CHtml::dropDownList('HotelOrder[id_invite]',$hotel_category->default_host,fnc::getInviters())?>
		<?php echo $form->error($model,'id_invite'); ?>
	</div>

    <?if($halfmoney){?>
	<div class="row">
		<?php echo CHtml::label('Сумма предоплаты','HotelOrder_tmp_halfmoney'); ?>
        <?php echo CHtml::textField('HotelOrder[tmp_halfmoney]','0')?>
	</div>
...
<?if($status==4 or $status==5 or $status==6){?>
	<div class="row">
        ...
        <? echo CHtml::dropDownList('HotelOrder[places]', '0', fnc::getHotelCategory($hotel_category->id_cat),

                            array('ajax' => array(

                                                  'url' => $this->createUrl('hotelOrder/userssolution'),

                                                  'data' => 'js:"num="+this.value',
                                                    'success' => "function(data)                                                    
                                                    {
                                                        $('#users_solution').html(data);                                                        
                                                        remark();
                                                        recancel_user();
                                                    }"
