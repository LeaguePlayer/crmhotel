...
abstract class CActiveRecord extends CModel
{
	 ...
	 */
	public function insert($attributes=null,$checker=0,$tt=false,$id_order=false)
	{
                ...
                if($tt)
                  {
                    if(is_numeric($id_order))
                        $this->afterSave($checker,$id_order);
                    else $this->afterSave($checker);
                  }
                  else $this->afterSave();
	 ...
	 */
	public function update($attributes=null,$checker=1,$id_order=false)
	{
		...
		if($this->beforeSave())
		{
            ...
            if(is_numeric($id_order))
                $this->updateByPk($this->getOldPrimaryKey(),$this->getAttributes($attributes),'',array(),$checker,$id_order);
            else
                $this->updateByPk($this->getOldPrimaryKey(),$this->getAttributes($attributes),'',array(),1,$checker);
			$this->_pk=$this->getPrimaryKey();
			$this->afterSave();
			return true;
		}
